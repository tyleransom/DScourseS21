...
players <- seasons_players(2021, nest_data = F, return_message=T)

salaries <- hoopshype_salaries()

stats <- metrics_leaders(seasons=2021, metric = "pts", modes = "PerGame")

final <- left_join(salaries,stats, by=("namePlayer")) %>% drop_na(amountContract,fgm) %>% filter(slugSeason.x=="2020-21")
