...
- Coding syntax for basic SQL operations

---
# SQL

- Structured Query Language (SQL) is a commonly language for processing relational databases 

- What is a relational database?

    - A system of databases
    
    - Most research questions involve more than one single database
    
    - The "ecosystem" of databases constitutes *relational data*
    
    - Manage data with *joins* (or *merges*)

- The next slides are a beginner's guide to using SQL (at the command line)

---
# Opening and Exiting out of SQL

- One can open SQL from the command line by typing `sqlite3` at the prompt

- SQL .hi[commands] are prepended with a `.` so to exit SQL, type `.quit`
...
- SQLite does not allow you to drop a column 

- The workaround is to make a new table that contains only the columns you want to keep, then rename the new table to the original template's name
...
- The way to save data is to issue the `.dump` command:
...
---
# Using SQL syntax in R

- It is possible to manipulate dataframes in R with SQL synatx
...
# dplyr way:
counted.dplyr <- df %>% filter(Species=="virginica") %>% count
```
...
---
# Alias Criminals Table As `C`, Select All Names From `C`
...
_Note: Use 'CREATE UNIQUE INDEX' if you want the index to not contain any duplicates_
