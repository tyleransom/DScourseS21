...
---
# `prep()` and `bake()` your `recipe()`

- With your recipe created, now you can apply it to the training and test sets

- Three steps:

    1. `prep()` your recipe
    2. Apply your "prepped" recipe to the training data with `juice()`
    3. Apply your "prepped" recipe to the testing data with `bake()`
...
rec_folds <- vfold_cv(housing_train_x %>% bind_cols(tibble(medv = housing_train_y)), v = 10)
```
